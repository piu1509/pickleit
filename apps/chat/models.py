...
from apps.user.helpers import send_push_notifications
...
@receiver(post_save, sender=NotificationBox)
def send_push_notification_on_create(sender, instance, created, **kwargs):
    if created:
        user = instance.notify_for
        if user:
            check_token = FCMTokenStore.objects.filter(user=user)
            if check_token.exists():
                try:
                    token_list = check_token.first().fcm_token.get("fcm_token", [])
                    if token_list:
                        send_push_notifications(token_list[0], instance.titel, instance.text_message)
                except:
                    pass
